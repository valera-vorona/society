...
void *nk_sdl_device_upload_image(const char *fname, SDL_TextureAccess access) {
    ...
    SDL_UpdateTexture(g_SDLTexture, NULL, image, 4 * width);
...
int get_image_size(SDL_Texture *image, int *x, int *y) {
    int rv = SDL_QueryTexture(image, NULL, NULL, x, y);
    if (rv)
        SDL_Log("Can't get texture size: %s", SDL_GetError());
...
nk_sdl_init(SDL_Window *win, SDL_Renderer *renderer, void *userdata)
{
    ...
    switch(evt->type)
    {
        ...
        case SDL_MOUSEBUTTONDOWN:
            {
                ...
                switch(evt->button.button)
                {
                    ...
                    case SDL_BUTTON_RIGHT:
                                            ...
                                            ctx->input.mouse.grab = down;
                                            ctx->input.mouse.ungrab = !down;
                                            ctx->input.mouse.grabbed = down;
                                            break;
                }
            }
            return 1;
        ...
        case SDL_MOUSEMOTION:
            if (ctx->input.mouse.grabbed) {
                ...
                app_scroll_map(sdl.userdata, -evt->motion.xrel, -evt->motion.yrel);
            }
        ...
        case SDL_MOUSEWHEEL:
            ...
            app_zoom(sdl.userdata, evt->wheel.x, evt->wheel.y);
            return 1;
    }
    return 0;
}
...
void nk_sdl_shutdown(void)
{
    ...
    for (int i = 0, ie = arrlenu(dev->images); i != ie; ++i)
        SDL_DestroyTexture(dev->images[i]);
    arrfree(dev->images);
